...
function isContainNumber(num, digit) {
  const absoluteDigit = Math.abs(digit);
  let remainingNum = Math.abs(Math.floor(num));

  while (remainingNum > 0) {
    const currentDigit = remainingNum % 10;
    if (currentDigit === absoluteDigit) {
      return true;
    }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = i; j < n; j += 1) {
      [nMatrix[i][j], nMatrix[j][i]] = [nMatrix[j][i], nMatrix[i][j]];
    }
  }
  for (let i = 0; i < n; i += 1) {
...
function sortByAsc(arr) {
  function partition(nArr, low, high) {
    const pivot = nArr[high];
    let i = low - 1;
    const arr2 = arr;
  ...
  function quickSort(arr3, low, high) {
    if (low < high) {
      const pi = partition(arr3, low, high);
...
function shuffleChar(str, iterations) {
  ...
  function getShuffle() {
    ...
    newStr = start + oddElement;
  }
  do {
    getShuffle();
    count += 1;
  } while (newStr !== str);
  const min = iterations % count;
  newStr = str;
